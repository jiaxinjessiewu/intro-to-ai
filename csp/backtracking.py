    ...
    for val in var.curDomain():
        ...
        for constraint in csp.constraintsOf(var): 
            if constraint.numUnassigned() == 1:
                if FCCheck(constraint,var,val) == "DWO": #prune future variables 
        ...
        if len(solns) > 0 and not allSolutions:
                break #don't bother with other values of var
                      #as we found a soln.
        Variable.restoreValues(var, val) #restore values pruned by this assignment 
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    while not len(constraints) == 0:
        cnstr = constraints.pop() #make cnstr GAC
                ...
                if not cnstr.hasSupport(var,val): 
                    ...
                    for recheck in csp.constraintsOf(var):
                        if recheck != cnstr and not recheck in constraints: 
                            constraints.insert(len(constraints), recheck)
    return "OK"
