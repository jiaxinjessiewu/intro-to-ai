...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ''' qi and qj are objects of type CSP found in csp.py '''
        self._name = "Queen_" + str(name)
        scope = [qi, qj]
        self.i = i
        self.j = j
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.isAssigned() and v.getValue() == self._required:
                assignments.append(v.getValue())
        return (len(assignments)>=self._lb and len(assignments)<=self._ub)
    ...
    def hasSupport(self, var, val):
        ...
        def meetlbub(l):
            vals = [val for (var, val) in l]
            vals = filter(lambda x: x==self._required,vals)
            return self._lb<=len(vals) and len(vals)<=self._ub

        def partialMeetlbub(l):
            vals = [val for (var, val) in l]
            vals = filter(lambda x: x==self._required,vals)
            return self._ub>=len(vals)
    
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], meetlbub, partialMeetlbub)
        return x
