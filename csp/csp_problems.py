...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.extend(post_all_pairs_diff(row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.extend(post_all_pairs_diff(scope))
...
def post_all_pairs_diff(var_list):
    '''create a not equal constraint between all pairs of variables in var_list
        return list of constructed constraint objects'''
    constraints = []
    for i in range(len(var_list)):
        for j in range(i+1,len(var_list)):
            for a in range(j+1,len(var_list)):
                for b in range(a+1,len(var_list)):
                    for c in range(b+1,len(var_list)):
                        for d in range(c+1,len(var_list)):
                            for e in range(d+1,len(var_list)):
                                for f in range(e+1,len(var_list)):
                                    for g in range(f+1,len(var_list)):
                                        c = AllDiffConstraint("({},{},{},{},{},{},{},{},{})".format(var_list[i].name(), var_list[j].name(),var_list[a].name(), var_list[b].name(),var_list[c].name(), var_list[d].name(),var_list[e].name(), var_list[f].name(), var_list[g].name()),[var_list[i], var_list[j],var_list[a], var_list[b],var_list[c], var_list[d],var_list[e], var_list[f],var_list[g]])
                                        constraints.append(c)
    return constraints
